															...
															const WholeBodyState& state)
{
	...
	constraint.resize(fbs_->getJointDoF() + 3 * num_actived_endeffectors_);
	...
	wdyn_->computeConstrainedFloatingBaseInverseDynamics(estimated_joint_forces,
														 state.base_pos, state.joint_pos,
														 state.base_vel, state.joint_vel,
														 base_acc, joint_acc,
														 active_endeffectors_);
	constraint.segment(0, fbs_->getJointDoF()) = estimated_joint_forces - state.joint_eff;
	...
	wkin_->computeVelocity(endeffectors_vel,
						   state.base_pos, state.joint_pos,
						   state.base_vel, state.joint_vel,
	...
	for (rbd::BodyVectorXd::iterator endeffector_it = endeffectors_vel.begin();
		...
		constraint.segment<3>(fbs_->getJointDoF() + 3 * id) = endeffector_it->second;
	}
}
...
{
	lower_bound = Eigen::VectorXd::Zero(fbs_->getJointDoF() + 3 * num_actived_endeffectors_);
	upper_bound = Eigen::VectorXd::Zero(fbs_->getJointDoF() + 3 * num_actived_endeffectors_);
}
