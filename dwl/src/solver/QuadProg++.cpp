...
inline void compute_d(Eigen::VectorXd &d,
...
inline void update_z(Eigen::VectorXd& z,
					 const Eigen::MatrixXd& J,
...
inline void update_r(const Eigen::MatrixXd& R,
					 Eigen::VectorXd& r,
					 const Eigen::VectorXd& d, int iq)
{
	r.head(iq) = R.topLeftCorner(iq, iq).triangularView<Eigen::Upper>().solve(
			d.head(iq));
}


inline double solve_quadprog(Eigen::MatrixXd & G, Eigen::VectorXd & g0,
...
inline bool add_constraint(Eigen::MatrixXd& R, Eigen::MatrixXd& J,
						   Eigen::VectorXd& d, int& iq,
						   double& R_norm)
{
	int n = J.rows();
#ifdef TRACE_SOLVER
...
inline void delete_constraint(Eigen::MatrixXd& R, Eigen::MatrixXd& J,
