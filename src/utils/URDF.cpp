				   ...
				   rbd::FloatingBaseSystem* system)
{
	...
	unsigned joint_idx = 0, virtual_joints_idx = 0;
					 ...
					 rbd::FloatingBaseSystem* system)
{
	...
	JointID joints;
	getJointNames(joints, urdf_model, system);
	...
	for (urdf_model::JointID::iterator jnt_it = joints.begin();
		...
		if (current_joint->type == urdf::Joint::FIXED) {
			boost::shared_ptr<urdf::Link> root_link = model->links_[model->getRoot()->name];
					...
					rbd::FloatingBaseSystem* system)
{
	...
	for (urdf_model::JointID::iterator jnt_it = joints.begin();
		...
		if (current_joint->type == urdf::Joint::PRISMATIC ||
				current_joint->type == urdf::Joint::REVOLUTE) {
			unsigned int joint_idx = jnt_it->second;
