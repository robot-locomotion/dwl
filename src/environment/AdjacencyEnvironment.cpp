...
{
	printf(
			BLUE "Setting the robot information in the %s adjacency model \n" COLOR_RESET,
			name_.c_str());
	robot_ = robot;

	printf(
			BLUE "Setting the environment information in the %s adjacency model \n" COLOR_RESET,
			name_.c_str());
...
void AdjacencyEnvironment::computeAdjacencyMap(AdjacencyMap& adjacency_map,
		Vertex source, Vertex target)
{
	printf(
			YELLOW "Could compute the whole adjacency map because it was not defined an adjacency model\n" COLOR_RESET);
}


void AdjacencyEnvironment::getSuccessors(std::list<Edge>& successors,
		Vertex state_vertex)
{
	printf(
			YELLOW "Could get the successors because it was not defined an adjacency model\n" COLOR_RESET);
}


void AdjacencyEnvironment::getTheClosestStartAndGoalVertex(
		Vertex& closest_source, Vertex& closest_target, Vertex source,
		Vertex target)
{
	...
	if ((!is_there_start_vertex) && (!is_there_goal_vertex))
	{
		...
		for (unsigned int i = 0; i < vertex_map.size(); i++)
		{
			...
			environment_->getTerrainSpaceModel().vertexToState(current_state,
					vertex_map[i]);

			// Calculating the distant to the vertex from start and goal positions
			double start_distant =
					(start_state.head(2) - current_state.head(2)).norm();
			double goal_distant =
					(goal_state.head(2) - current_state.head(2)).norm();
...
void AdjacencyEnvironment::getTheClosestVertex(Vertex& closest_vertex,
		Vertex vertex)
{
...
double AdjacencyEnvironment::heuristicCostEstimate(Vertex source, Vertex target)
{
	...
	double heuristic = (5 * distance + 2.5 * dist_orientation)
			* uncertainty_factor_
			* (environment_->getAverageCostOfTerrain() + 0.1); //TODO Tunning the heuristic function
...
bool AdjacencyEnvironment::isReachedGoal(Vertex target, Vertex current)
{
	if (isLatticeRepresentation())
	{
		...
		environment_->getTerrainSpaceModel().vertexToState(current_state,
				current);
		environment_->getTerrainSpaceModel().vertexToState(target_state,
				target);
		...
		double distant = ((Eigen::Vector2d) target_state.head(2)
				- (Eigen::Vector2d) current_state.head(2)).norm();
		if (distant < epsilon)
		{
			double angular_error = (double) target_state(2)
...
bool AdjacencyEnvironment::isFreeOfObstacle(Vertex state_vertex,
		TypeOfState state_representation, bool body)
{
	return true;
}
