...
/*	initial_pose_ = start;
	...
	goal_vertex_ = gridmap_.coordToVertex((Eigen::Vector2d) goal.position.head(2));
}


bool HierarchicalPlanning::compute(Pose current_pose)
{
	Vertex current_vertex = gridmap_.coordToVertex((Eigen::Vector2d) current_pose.position.head(2));
